  ...
  methods: {
    ...
    animateDragDirection() {
      ...
      if (movedBy > 50 && state.indices.curIndex > 0) {
        ...
        movedBy < -50 &&
        state.indices.curIndex < state.slides.length - 1
      ) {
        curObj = { scale: 1, transition, duration };
    ...
    touchStart(event) {
      ...
      if (state.drag.start && !state.drag.dragging && !state.drag.animating) {
        ...
        animate.to(curSlide, { scale: 0.98, ease: "ease", duration: 0.2 });
      ...
      if (state.drag.dragging) {
        ...
        this.stateObjs({
          dragPositions: {
            ...
            curPos:
              state.dragPositions.prevPos +
              curDragPos -
              state.dragPositions.startPos,
          },
          drag: { dragged: true },
        });
    ...
    touchEnd() {
      ...
      const animate = gsap.timeline();
      const curSlide = state.slides[state.indices.curIndex].value;
      animate.to(curSlide, { scale: 1.05, ease: "ease", duration: 0.2 });

  ...
  getters: {
    ...
    totalComputedValues: (state, getters) => (el, props) => {
      ...
      for (const cur of props) {
        const val = getters.removeUnit(
          computedProps.getPropertyValue(`${cur}`)
        );
        width += val;
      }
      return width;
  ...
  actions: {
    ...
    slideIndices({ state, getters, commit }, mode) {
      ...
      if (options.offset && options.loop) {
        if (offset.initial)
          trails = getters.trailingIndices("decr", indices.offset);
    ...
    sliderEndPos({ state, getters, commit }, container) {
      ...
      for (const slide of slides) {
        ...
        width += getters.totalComputedValues(curSlide, [
          "border-left-width",
          "border-right-width",
        ]);
        width += getters.totalComputedValues(curSlide, [
    ...
    sliderCurPos({ state, getters, commit }, trails) {
      ...
      for (const cur of trails) {
        ...
        width += getters.totalComputedValues(slide, [
          "margin-left",
          "margin-right",
        ]);
      }
